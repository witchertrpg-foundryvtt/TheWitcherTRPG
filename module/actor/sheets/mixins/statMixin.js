    ...
    async _onReputation(event) {
        ...
        if (this.actor.system.reputation.modifiers.length > 0) {
            ...
            this.actor.system.reputation.modifiers.forEach(
                (mod) =>
                    (dialogTemplate += `<div><input id="${mod.name.replace(/\s/g, '')}" type="checkbox" unchecked/> ${
                        mod.name
                    }(${mod.value})</div>`)
            );
        }
            ...
            buttons: {
                t1: {
                    ...
                    callback: async (html) => {
                        ...
                        let messageData = { speaker: ChatMessage.getSpeaker({ actor: this.actor }) };
                        messageData.flavor = `
                <h2>${game.i18n.localize('WITCHER.ReputationTitle')}: ${game.i18n.localize(
                            'WITCHER.ReputationSave.Title'
                        )}</h2>
                <div class="roll-summary">
                  <div class="dice-formula">${game.i18n.localize('WITCHER.Chat.SaveText')}: <b>${statValue}</b></div>
                </div>
                ...
                t2: {
                    ...
                    callback: async (html) => {
                        ...
                        let messageData = { speaker: ChatMessage.getSpeaker({ actor: this.actor }) };
                        let rollFormula = `1d10 + ${Number(repValue)}[${game.i18n.localize(
                            'WITCHER.Reputation'
                        )}] + ${Number(this.actor.system.stats.will.current)}[${game.i18n.localize('WITCHER.StWill')}]`;
                        messageData.flavor = `
                <h2>${game.i18n.localize('WITCHER.ReputationTitle')}: ${game.i18n.localize(
                            'WITCHER.ReputationFaceDown.Title'
                        )}</h2>
                <div class="roll-summary">
